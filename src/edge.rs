...
use log::warn;
use crate::{config, data_collection, destinations};
use crate::html::{parse_html, Document};
use crate::tools::edgee_cookie;
use crate::entrypoint::{set_edgee_header};
    ...
    match do_process_payload(&path, request_headers, response_headers) {
        Ok(_) => {
            ...
            if cookie.is_none() {
                ...
                let payload = data_collection::process_document(&document, &cookie.unwrap(), proto, &host, &path, &response_headers, remote_addr);
