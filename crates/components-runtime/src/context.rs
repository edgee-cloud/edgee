...
use crate::data_collection::data_collection_0_5_0::{
    DataCollectionOFiveO, DataCollectionOFiveOPre,
};
use crate::data_collection::data_collection_1_0_0::{DataCollectionOne, DataCollectionOnePre};
...
pub struct Components {
    pub data_collection_0_5_0: HashMap<String, DataCollectionOFiveOPre<HostState>>,
    pub data_collection: HashMap<String, DataCollectionOnePre<HostState>>,
...
pub fn pre_instanciate_data_collection_component_internal(
...
) -> anyhow::Result<DataCollectionOnePre<HostState>> {
    ...
    let instance_pre = DataCollectionOnePre::new(instance_pre)?;
...
pub fn pre_instanciate_data_collection_0_5_0_component_internal(
    engine: &Engine,
    component_config: &DataCollectionComponents,
) -> anyhow::Result<DataCollectionOFiveOPre<HostState>> {
    let mut linker = Linker::new(engine);
    wasmtime_wasi::add_to_linker_async(&mut linker)?;
    ...
    let component = Component::from_file(engine, &component_config.file)?;
    let instance_pre = linker.instantiate_pre(&component)?;
    let instance_pre = DataCollectionOFiveOPre::new(instance_pre)?;
...
impl ComponentsContext {
    pub fn new(config: &ComponentsConfiguration) -> anyhow::Result<Self> {
        ...
        let data_collection_components = config
            .data_collection
            .iter()
            .filter(|entry| entry.version == DataCollectionProtocolVersion::V1_0_0)
            .map(|entry| {
                let instance_pre =
        ...
        let data_collection_0_5_0_components = config
            .data_collection
            .iter()
            .filter(|entry| entry.version == DataCollectionProtocolVersion::V0_5_0)
            .map(|entry| {
                let instance_pre =
    ...
    pub fn add_data_collection_component(
        &mut self,
        component_config: DataCollectionComponents,
        instance_pre: DataCollectionOnePre<HostState>,
    ) {
        if !self
            .components
            .data_collection
    ...
    ) -> anyhow::Result<DataCollectionOne> {
    ...
    pub async fn get_data_collection_0_5_0_instance(
        ...
        id: &str,
        store: &mut Store<HostState>,
    ) -> anyhow::Result<DataCollectionOFiveO> {
