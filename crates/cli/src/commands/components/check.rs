        ...
        ComponentType::DataCollection => match component_wit_version {
            "0.5.0" => ComponentsConfiguration {
                data_collection: vec![DataCollectionComponents {
                    ...
                    version: edgee_components_runtime::data_collection::version::DataCollectionProtocolVersion::V0_5_0,
                    ..Default::default()
                }],
                ..Default::default()
            },
            "1.0.0" => ComponentsConfiguration {
                data_collection: vec![DataCollectionComponents {
                    id: component_path.to_string(),
                    file: component_path.to_string(),
                    version: edgee_components_runtime::data_collection::version::DataCollectionProtocolVersion::V1_0_0,
                    ..Default::default()
                }],
                ..Default::default()
            },
...
pub async fn run(_opts: Options) -> anyhow::Result<()> {
    ...
    let (component_path, component_type, component_wit_version) = match (
        ...
        (Some(filename), Some(component_type), Some(version)) => {
            (filename, ComponentType::DataCollection, version)
        }
        _ => {
