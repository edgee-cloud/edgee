...
use edgee_components_runtime::data_collection::versions::v1_0_1::data_collection::exports::edgee::components1_0_1::data_collection::EdgeeRequest;
use edgee_components_runtime::data_collection::versions::v1_0_1::data_collection::exports::edgee::components1_0_1::data_collection::AuthRequest;
use edgee_components_runtime::data_collection::versions::v1_0_1::data_collection::exports::edgee::components1_0_1::data_collection::HttpMethod;
...
async fn run_auth_request(request: AuthRequest) -> anyhow::Result<(String, String)> {
    use std::str::FromStr;
    ...
    match res {
        Ok(res) => {
            ...
            match pretty_json {
                Ok(pretty_json) => {
                    ...
                    match request.response_token_property_name {
                        Some(property_name) => {
                            if let Some(token) = pretty_json.get(&property_name) {
                                Ok((
                                    request.component_token_setting_name.clone(),
                                    token.to_string(),
                                ))
                            } else {
                                Err(anyhow::anyhow!(
                                    "Response token property '{}' not found in response body",
                                    property_name
                                ))
                            }
                        }
        ...
        Err(e) => Err(anyhow::anyhow!("Failed to make auth request: {}", e)),
    }
}
...
pub async fn test_data_collection_component_1_0_1(
    opts: super::Options,
    manifest: &Manifest,
) -> anyhow::Result<()> {
    ...
    let mut settings = settings_map.clone().into_iter().collect();
    ...
    // call auth if needed
    let auth_request = match component.call_authenticate(&mut store, &settings).await {
        Ok(Ok(request)) => request,
        Err(e) => return Err(anyhow::anyhow!("Failed to call auth: {e}")),
        Ok(Err(e)) => return Err(anyhow::anyhow!("Failed to call auth: {e}")),
    };
    ...
    if let Some(auth_request) = auth_request {
        ...
        if opts.make_http_request {
            let Ok(token) = run_auth_request(auth_request).await else {
                return Err(anyhow::anyhow!("Failed to run auth request"));
            };
            settings.push(token);
        } else {
            settings.push((
                auth_request.component_token_setting_name,
                "auth_token_placeholder".to_string(),
            ));
        }
    } else {
        tracing::info!("No auth request needed.");
    }
